...
app.post("/login", async (req, res) => {
    try {
      ...
      const token = jwt.sign(
        { id: user.recordset[0].id, role: user.recordset[0].role },
      ...
      await pool
        ...
        .query("INSERT INTO user_sessions (user_id, login_time, token) VALUES (@user_id, GETDATE(), @token)");
  ...
  app.post("/logout", async (req, res) => {
    try {
      ...
      await pool
        ...
        .input("user_id", sql.Int, userId)
        .input("token", sql.VarChar, token)
        .query("UPDATE user_sessions SET logout_time = GETDATE() WHERE user_id = @user_id AND token = @token");
